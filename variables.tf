...
variable "retention_in_days" {
  description = "(Optional) The retention in days for the logs in the Log Analytics Workspace."
  type        = number
  default     = null
}
...
variable "table" {
  ...
  description = "(Optional) The table's data of log analytics"
  validation {
    condition     = var.table != [] ? alltrue([for t in var.table : contains(["Analytics", "Basic"], t.plan)]) : true
    error_message = "The plan must be either 'Analytics' or 'Basic'."
  }
  default = []
}
...
variable "analytics_diagnostic_monitor_lwk_name" {
  type        = string
  description = "(Optional) Specifies the name of a Log Analytics Workspace where Diagnostics Data should be sent."
  default     = null
}

variable "analytics_diagnostic_monitor_lwk_rsg" {
  type        = string
  description = "(Optional) The name of the resource group where the lwk is located. Only applies if analytics_diagnostic_monitor_enabled is true. If not set, it assumes the rsg_name value."
  default     = null 
}
